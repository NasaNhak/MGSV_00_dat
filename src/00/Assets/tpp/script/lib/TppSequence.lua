...
if t=="boolean"
then
s[e]={name=i,type=TppScriptVars.TYPE_BOOL,value=n,save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}
elseif t=="number"
then
s[e]={name=i,type=TppScriptVars.TYPE_INT32,value=n,save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}
elseif t=="string"
then
s[e]={name=i,type=TppScriptVars.TYPE_UINT32,value=o(n),save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}
elseif t=="table"
then
s[e]=n
end
...
i.Seq_Mission_Prepare={Messages=function(e)
...
{msg="StartMissionTelopFadeIn",func=function()n("Timer_HelicopterMoveStart",s)
end,option={isExecMissionPrepare=true,isExecMissionClear=true,isExecGameOver=true}
},
...
i.Seq_Mission_Prepare.HelicopterMoveStart()n("Timer_FadeInStartOnNoTelopHelicopter",t)
else
...
{name="dbg_seq_sequenceName",type=TppScriptVars.TYPE_UINT32,value=0,save=true,sync=true,wait=true,category=TppScriptVars.CATEGORY_MISSION},nil}
end
function e.DEBUG_Init()
end
function e.Init(n)
e.MakeSequenceMessageExecTable()svars.seq_sequence=e.GetSequenceIndex"Seq_Mission_Prepare"
if n.sequence then
...
if e.debug.showCurrentSequence then(nil).Print(n,{.5,.5,1},"LuaSystem SEQ.showCurrSequence");(nil).Print(n," current_sequence = "..tostring(a(s.seq_sequence)))
end

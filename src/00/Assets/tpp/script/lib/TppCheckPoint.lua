...
{name="chk_checkPointEnable",arraySize=TppDefine.CHECK_POINT_MAX,type=TppScriptVars.TYPE_BOOL,value=0,save=true,sync=false,wait=false,category=TppScriptVars.CATEGORY_MISSION},nil}
end
function e.Messages()
...
local n=e.Messages()
if n then
e.messageExecTable=Tpp.MakeMessageExecTable(n)
end
end
function e.OnReload()
local n=e.Messages()
...
function e.OnMessage(t,c,n,i,o,a,s)
Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,t,c,n,i,o,a,s)
end
function e.Enable(n)
e._SetEnable(n,true)
end
function e.Disable(n)
e._SetEnable(n,false)
end
function e.Reset()
gvars.mis_checkPoint=s
end
function e.RegisterCheckPointList(n)
local n=n or{
}
for t,n in pairs(n)do
e._RegisterCheckPoint(n)
end
...
function e.SetCheckPointPosition()
...
local e,n=e.GetCheckPointLocator(n)
if e then
TppPlayer.SetInitialPosition(e,n)
end
end
function e.GetCheckPointLocator(e)
if not t(e)then
return
end
return Tpp.GetLocator("CheckPointIdentifier",e.."_Player")
end
function e.FindNearestCheckPoint(n)
if not i(n)then
return
end
...
if t(n)then
local e,t=e.GetCheckPointLocator(n)
if e then
...
e.Reset()
TppPlayer.SetInitialPositionToCurrentPosition()Player.NotifyCheckPointTrapName(l)
...
if mvars.debug.showCheckPointList then(nil).Print(i,{.5,.5,1},"TppCheckPoint: showCheckPointList")
for o,n in pairs(mvars.mis_checkPointList)do
if t(n)and e.IsEnable(n)then(nil).Print(i,{1,1,1},n)
end
end
end
end
function e._SetEnable(n,o)
if not n then
return
end
if i(n)then
...
function e._RegisterCheckPoint(n)
if not n then
return
end
...
function e._DoesBaseListInclude(n)
...
function e._DoesCheckPointListInclude(e)
if not e then
return false
end
local n

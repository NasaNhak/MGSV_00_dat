...
e.ShowCallSupportBuddyAnnounceLog()
end
end}
}
}
...
TppSoundDaemon.ResetMute"Outro"p(t,i,o)
...
function e.UnsetOverrideFadeInGameStatus()
mvars.ui_onEndFadeInOverrideExceptGameStatusTemporary=nil
end
function e.GetOverrideGameStatus()
return mvars.ui_onEndFadeInOverrideExceptGameStatusTemporary
end
function e.SetFadeColorToBlack()FadeFunction.SetFadeColor(0,0,0,255)
end
function e.SetFadeColorToWhite()FadeFunction.SetFadeColor(255,255,255,255)
end
...
local i=e.ANNOUNCE_LOG_TYPE[i]
local e=e.ANNOUNCE_LOG_TYPE[o]
if i and e then
if n then
TppUiCommand.AnnounceLogDelayTime(n)
end
...
e.ShowAnnounceLog"emergencyMissionOccur"
...
function e.DisableMissionPhoto(e,n)
if TppUiCommand.IsMissionPhotoIdEnable(e)then
if n then
...
function e.EnableMissionSubGoal(e)
TppUiCommand.SetCurrentMissionSubGoalNo(e)
end
function e.EnableSpySearch(e)
if not i(e)then
return
end
local n,i
...
function e.StartMissionTelop(e,i,n)
TppSoundDaemon.SetMute"Telop"
if e then
TppUiCommand.SetMissionStartTelopId(e)
end
TppUiCommand.CallMissionStartTelop(i,n)
...
TppChallengeTask.RequestUpdate"MISSION_TASK"e.UpdateOnlineChallengeTask{detectType=(34+i),diff=1}
...
function e.IsTaskLastCompleted(n)
local e=e.GetLastCompletedFlagIndex(n)
if e then
return gvars.ui_isTaskLastComleted[e]
end
end
function e.SetTaskLastCompleted(n,i)
local e=e.GetLastCompletedFlagIndex(n)
if e then
gvars.ui_isTaskLastComleted[e]=i
end
end
function e.GetLastCompletedFlagIndex(n)
return e._GetLastCompletedFlagIndex(vars.missionCode,n)
end
function e._GetLastCompletedFlagIndex(n,e)
...
function e.GetTaskCompletedNumber(e)
...
function e.IsAllTaskCompleted(n)
local i=e.GetTaskCompletedNumber(n)
local e=e.GetMaxMissionTask(n)
if e==nil then
return false
end
if i>=e then
...
if not s then
if TppUiCommand.IsDispGuide"TipsGuide"
then
return
end
end
...
function e.IsEnableToShowGuide(e)
...
function e.StartDisplayTimer(e)
if not i(e)then
return
end
...
function e.ShowSavingIcon(e)
TppUiCommand.CallSaveMessage(e)
end
function e.ShowLoadingIcon()
TppUiCommand.CallLoadMessage()
end
function e.ShowAccessIcon()
TppUiCommand.CallAccessMessage()
end
function e.ShowAccessIconContinue()
TppUiCommand.CallAccessContinueMessage()
end
function e.HideAccessIcon()
TppUiCommand.HideAccessMessage()
end
function e.ShowTextureLogo()
TppUiCommand.ShowTextureLogo()
end
function e.HideTextureLogo()
TppUiCommand.HideTextureLogo()
end
function e.ShowCallSupportBuddyAnnounceLog()
local n=e.BUDDY_LANG_ID[mvars.ui_callSupportBuddyType]
if n then
local i=TppSupportRequest.GetCallBuddyGmpCost(mvars.ui_callSupportBuddyType)
e.ShowAnnounceLog("callSupportBuddyReceived",n)
...
function e.SetSupportCallBuddyType(e)
mvars.ui_callSupportBuddyType=e
end
function e.StartLoadingTips()
TppUiCommand.StartLoadingTipsCommon()
end
function e.FinishLoadingTips()
TppUiCommand.RequestEndLoadingTips()
end
function e.StartLyricOkb(e)
...
elseif n then
e.RegisterFobSneakPauseMenuPage()
else
...
elseif n then
e.RegisterFobSneakGameOverMenuItems()
else
...
function e.RegisterHeliSpacePauseMenuPage(n)
...
function e.RegisterFobSneakPauseMenuPage()
local e
...
function e.RegisterFobSneakGameOverMenuItems()
...
function e.EnableGameStatusOnFadeInStart()
...
function e.EnableGameStatusOnFade()
local e,n
...
function e._RegisterDefaultLandPoint()
